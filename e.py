...
def email(x,y):
    ...
    class entity_add:
        ...
        def write(self):
            f = open(f"C:\\Users\\varun sharma\\Desktop\\personal\\email program\\{x}.txt", "w+")
            n = self.name
            e = self.email
            p = self.password
    ...
    class entity_display:
        ...
        def delete(self):
            ...
            if answer == 'yes':
                ...
                f = open(f"C:\\Users\\varun sharma\\Desktop\\personal\\email program\\{x}.txt", 'r')
                lines = f.readlines()
                f.close()
    ...
    def onsubmit():
        ...
        p = password.get()
        n = name.get()
        x=messagebox.askquestion('Add Entity', 'Do you want to save this entry?, \n' + 'Name: ' + n + '\nEmail: ' + m + '\nPassword: ' + p)
        if x=="yes":
